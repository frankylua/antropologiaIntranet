(function ($) {
    ...
    function datosFormulario() {
        ...
        if (datos.inst_pasant === 'otro') { const nombre = String($('#pasantia_nueva_institucion').val() || '').trim(); if (!nombre) { mensaje('Ingrese una Institución válida.', 'danger'); return null; } datos.inst_pasant = '0'; datos.institucion_nueva = nombre; idInstitucionContextual = null; } else idInstitucionContextual = null;
        const institucionValida = idValido(datos.inst_pasant) || (datos.inst_pasant === '0' && datos.institucion_nueva);
        if (!institucionValida || !idValido(datos.pais_pasant) || !datos.prof_patr || !datos.fondo || !datos.ciudad || !datos.fech_in || !datos.fech_ter) { mensaje('Complete todos los campos requeridos.', 'danger'); return null; }
